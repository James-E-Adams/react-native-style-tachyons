...
function setStyles(props, clsPropName) {
  ...
  const fontSize = _.find(_.keys(typeScale), fSetting =>
    _.includes(splitted, fSetting)
  )
  ...
  for (let i = 0; i < splitted.length; i++) {
    ...
    if (cls.length > 0) {
      if (styles[cls]) {
        ...
        newProps.style.push({
          lineHeight:
            lineHeights[cls.replace(/_/gu, "-")] * styles[fontSize].fontSize
        })
...
function calculateStyle(elementsTree) {
  const { props } = elementsTree
  const { clsPropName, shouldRecurse } = options
  let newProps = null
  ...
  } else if (shouldRecurse && React.isValidElement(newChildren)) {
    /* Convert single child */
    const converted = calculateStyle(newChildren)
  ...
  if (translated) {
    ...
    return React.cloneElement(elementsTree, newProps)
  }
